...
.DESCRIPTION
    Automates common Microsoft 365 user offboarding tasks using Microsoft Graph
    and Exchange Online PowerShell modules. The script supports both interactive
    and app-only authentication and can process single or multiple users via
    CSV input.

    Supported offboarding actions include:
      - Disabling user accounts
      - Resetting passwords
      - Removing group memberships and admin roles
      - Revoking app role assignments
      - Hiding users from the GAL
      - Managing Exchange mailbox settings
      - Converting mailboxes to shared
      - Removing licenses
      - Signing users out of all active sessions

.PARAMETER TenantId
    Azure AD tenant ID used for app-only authentication.

.PARAMETER ClientId
    Azure AD application (client) ID used for app-only authentication.

.PARAMETER CertificateThumbprint
    Certificate thumbprint associated with the Azure AD application.

.PARAMETER CSVFilePath
    Path to a CSV file containing UserPrincipalName values for bulk offboarding.

.PARAMETER UPNs
    Comma-separated list of user principal names to offboard.
...
.EXAMPLE
    .\M365-Offboarding.ps1 -UPNs user1@contoso.com

.EXAMPLE
    .\M365-Offboarding.ps1 -CSVFilePath .\UsersToOffboard.csv

.EXAMPLE
    .\M365-Offboarding.ps1 -TenantId "<TenantId>" `
                           -ClientId "<ClientId>" `
